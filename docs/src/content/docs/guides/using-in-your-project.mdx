...
```bash
# Repository-level one-liner with PATH aliases (like setup-pixi)
bash scripts/restore.sh sandbox/developer-linux-64
# or
pixi run sandbox-restore

# Manual: the extracted branch has its executable at the root
...
/tmp/sb/pixi-sandbox doctor --branch-location /tmp/sb --verify
/tmp/sb/restore.sh .
