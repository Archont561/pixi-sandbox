...
| `--self-bin` | Release binary to embed under `.pixi-sandbox/tools/<platform>/pixi-sandbox` and copy to the branch root as `./pixi-sandbox` (`.exe` on Windows) |
...
Airlock restore — verify then materialize. A packed branch puts the self-binary at its root,
so the explicit form is convenient even when no `pixi-sandbox` is installed globally:
...
The generated `./restore.sh <project>` wrapper calls that root binary with `--force`; on
Windows use `.\restore.ps1 <project>` and `.\pixi-sandbox.exe`.

When invoked with no subcommand from a branch checkout, `./pixi-sandbox` does **not** write:
it runs `doctor --verify` and prints the restore command. This avoids guessing where the project
should be written.
...
For an extracted branch, `./pixi-sandbox` runs the read-only doctor/hint flow and
`./restore.sh <project>` calls the root binary. The repository-level script does fetch + worktree +
doctor + restore + PATH alias wiring (like setup-pixi's `pixi()` function + tools + dev bin).
