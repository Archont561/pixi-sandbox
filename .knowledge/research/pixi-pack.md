...
verified:
  ...
  - { by: process:sandbox-measurement, at: 2026-09-19T11:30:00Z }   # §16.8 — full pipeline run end-to-end on the current host
...
* ⚠️ ~~`api.anaconda.org` returned `000` from this host on the same day, so "is `pixi-pack`/`pixi-unpack` also on
  ...
  assumption~~ — **resolved by §16.8, 2026-09-19**: both are on conda-forge (that `000` was the closed E2B box).
  `conda create --override-channels -c conda-forge pixi-pack=0.7.11 pixi-unpack=0.7.11` succeeded, so the
  conda-forge build **matches the GitHub release tag exactly** (v0.7.11 → `0.7.11`), and the §16.8 probe ran via
  that route, not `pixi global install`. The pixi-proxy spelling (`pixi pack` → pixi-pack) and `binary_relocation`
  questions for the two packages are now observed in §16.8.

### 16.8 Measured (2026-09-19): the full loop runs end-to-end on this host

The environment files quote this probe ([inventory §6](/environment/inventory.md#6-re-measurement-2026-09-19-the-host-this-corpus-is-now-edited-on),
[consequences §5](/environment/consequences.md), [network-model §4.4](/environment/network-model.md#44-re-measurement-2026-09-19-the-authoring-host-is-no-longer-the-e2b-airlock));
here is what actually ran, on the current authoring host (Codespace-style, open egress), all binaries from
conda-forge (`pixi` 0.80.0, `pixi-pack`/`pixi-unpack` 0.7.11), no network after the initial pack:

1. **Build a project:** `pixi init --format pixi` + `pixi add libcurl` + `pixi install` → solved and installed
   **19 packages** (libcurl + conda-forge deps: libgcc `16.2.0`, openssl `3.6.4`, ncurses, zstd, …) from a real
   solve, not a stub.
2. **Pack:** `pixi pack --output-file environment.tar` (pixi 0.80.0 auto-dispatches to `pixi-pack` on `PATH`).
   Result: **`environment.tar`, 29.77 MiB** (31 216 128 bytes). Layout confirmed against §16.4 verbatim:
   `channel/linux-64/*.conda` + `channel/noarch/` + `pixi-pack.json` (`"version": 1`, `"pixi-pack-version":
   "0.7.11"`, `"platform": "linux-64"`). Quirk worth recording: bare `pixi-pack pack` parses `pack` as the
   positional *manifest path* and errors (`manifest path does not exist at pack`) — the pixi proxy (`pixi pack`)
   builds the correct argv, so design docs should name the pixi spelling.
3. **Unpack (rung 3):** `pixi-unpack environment.tar -o <out>` extracted and rattler-installed all **19
   packages**, wrote `conda-meta/history` ✅, and laid down `activate.sh` at the output root. `source activate.sh`
   then serves the prefix's own binaries (`$ENV/bin` prepended, `CONDA_PREFIX` set) — e.g. `icuinfo` runs from
   `/out/env/bin/icuinfo` with the unpacked icu, not the host's.
4. **`file://` channel (rung 2):** a *consumer* project, pointed only at the pack's extracted `channel/`
   directory — `pixi project channel add file://…/channel` — then `pixi add libcurl --offline` and
   `pixi install --offline` resolved and installed against that local repodata with **no channel access**, and
   `pixi run icuinfo` executed in the reconstructed env. The §16.6 "single probe that still moves a default" is
   therefore **green**: a `file://` channel built from a real `pixi-pack` tarball serves `pixi install`
   offline, so **rung 2 is the advertised guarantee** and `cache/pkgs/` is an optional size optimisation.

Honest limits of this probe: it ran on this open host, not inside a sealed container (the §16.6 wording demanded
"a container with no network" — the stronger statement is still M1.5's); `pixi install --offline` emitted the
documented lockfile warning (*"written from a solve restricted to locally available packages"*), which is
exactly §16.1's documented behaviour, not a defect; and the *pack* step still consumed the network (it must —
that is the builder side, which lives in CI by design). Three unproven rows in
