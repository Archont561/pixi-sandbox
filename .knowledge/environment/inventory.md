...
verified:
  - { by: process:sandbox-measurement, at: 2026-09-18T21:20:00Z }
  - { by: process:sandbox-measurement, at: 2026-09-19T10:30:00Z }   # re-measurement §6 — E2B box gone, host changed, egress open
  - { by: process:sandbox-measurement, at: 2026-09-19T11:45:00Z }   # pixi 0.81.0 from the official installer script, on PATH
...
> [!WARNING]
> **Read the date on this file.** §1–§5 below describe the **E2B host** this bundle was authored on
> (measured 2026-09-18): a **GitHub + npm + PyPI sandbox, not a Rust/pixi sandbox**, where the Rust
> toolchain, crates.io, prefix.dev, conda channels, `pixi.sh` and GitHub release-asset downloads were all
> unreachable and `pixi`/`cargo` could only be *designed for*, never executed. That box is gone. The
> current authoring host (a GitHub Codespace-style box, re-measured **2026-09-19**) has **open egress and
> installs the whole stack from conda-forge** — see [§6 Re-measurement below](#6-re-measurement-2026-09-19-the-host-this-corpus-is-now-edited-on).

The E2B box in §1–§5 was a **GitHub + npm + PyPI sandbox, not a Rust/pixi sandbox**: `git`, `gh`, Node and
npm work (bun only *via* npm); the Rust toolchain, crates.io, prefix.dev and conda channels, `pixi.sh` and
GitHub release-asset downloads are all unreachable. So the `pixi`/`cargo` half of the
`pixi`-`cargo`-`bun` stack cannot be installed or executed there — it can only be *designed for* and
validated in CI. One measured exception worth remembering: **`bun` 1.4.2 installs fine via `npm i bun`**,
because its npm postinstall pulls the binary from `registry.npmjs.org` (see §3 below).

Every number in §1–§5 was measured on **2026-09-18 ~21:20 UTC** from inside the E2B sandbox; the commands
that produce them are collected in [Reproducing These Measurements](/environment/reproducing.md).
...
## 6. Re-measurement (2026-09-19): the host this corpus is now edited on

The E2B box above is closed; the current authoring host was re-probed on 2026-09-19 and **no longer shares
any of the constraints that shaped the E2B rows**. It is a GitHub Codespace-style box:

| Probe | Result |
|---|---|
| Host / paths | hostname `codespaces-…`; repo at `/workspaces/pixi-sandbox`; user `codespace` with `sudo` |
| `conda` | ✅ **26.7.2** at `/opt/conda/bin/conda` — conda-forge reachable, `conda create` installs work |
| `pixi` | ✅ **0.81.0** installed from the official installer script (`curl … install.sh \| bash` → `~/.pixi/bin/pixi`, musl static; added to `PATH` in `~/.bashrc`); `pixi --version`/`pixi info` green on this build. The `init/add/install/run`, `--list` and `--offline` verbs were exercised on **0.80.0** (conda-forge scratch env, §16.8); the documented route for this host is now the installer script |
| `rustc` / `cargo` | ✅ **1.98.1** (rustc build `48a229cea`, cargo `797e8a9bc`) installed from conda-forge into a scratch env and executed |
| `pixi-pack` / `pixi-unpack` | ✅ pinned to **0.7.11** by that conda env; `pixi --list` reports them as external subcommands (`pack (via pixi-pack)` / `unpack (via pixi-unpack)`) |
| `convco` | ✅ **0.7.2** from conda-forge; `convco check`/`changelog`/`config --default` executed against this repo |
| egress | open — see [Network Model §4.4](/environment/network-model.md#44-re-measurement-2026-09-19-the-authoring-host-is-no-longer-the-e2b-airlock) |

**One-line net change for the corpus:** the design's *target* (a git-only airlock) is unchanged and its
constraints still stand, but the "cannot be executed here" rows are **historical**. The core pipeline
(`pixi` → `pixi-pack` → `file://` channel → `pixi install/run`, and `pixi-unpack`) was executed
