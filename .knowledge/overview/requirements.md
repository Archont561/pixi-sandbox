...
| **(c)** decide what to bundle **from `Cargo.lock` / `bun.lock`** | `--components auto`: marker files *are* the spec — `pixi.lock`→env, `Cargo.lock`→vendor, own binary→self (a JS lockfile is *validated*, not packed — D20); absent marker ⇒ `not-applicable`, never an error | [§4.4](/spec/architecture.md#44-discovery-the-inventory), [§8](/spec/packagers.md#8-the-three-packagers), [D13](/spec/decisions.md#16-decision-log) |
