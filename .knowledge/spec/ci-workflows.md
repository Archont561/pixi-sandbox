...
jobs:
  checks-l0:                            # the only jobs runnable in the authoring sandbox today ✅
    steps:
      - { run: python3 ../val.py }      # until M1 moves it into the repo; expect: ALL CLEAN
...
* **Every YAML block in this file** 🚧 — no workflow exists yet; L0 (`val.py`, `npx js-yaml`) is the only
