...
| `docs` (`if: [docs].enabled`) | `pixi run docs-check` = sync → `astro build` → assert `pages ≥ *.md count` → `starlight-links-validator` | the validator fails the build on 104 findings ✅ measured — and it is the *only* checker here that follows `FILE.md#anchor` across files |
