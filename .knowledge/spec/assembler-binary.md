...
**Measured 2026-09-19 — `compressed-env` branch lesson**: prototype kit shipped only `pixi` binary, not pair `pixi`+`pixi-unpack`. Dev env contained `pixi-unpack` conda package but needed `pixi-unpack` to unpack dev env → chicken-egg. Fix: `bin/` must contain **pair** (D19 `ship-pixi` already says pair) and assembler must have fallback manual `.conda` extraction (zip → `pkg-*.tar.zst` → zstd decompress → `bin/pixi-unpack`) when pair missing, using bundled zstd or python `zstandard` fallback (pypi.org reachable even in airlock). Also chunked artifacts `.part_00` must be reassembled via `cat *.part_* > file` before verify — see [artifacts §9.5](/spec/artifacts.md#95-chunked-artifacts-for-githubs-100-mib-limit) and [pixi-pack §16.9](/research/pixi-pack.md#169-measured-2026-09-19-restoring-the-compressed-env-branch-in-an-airlocked-arena).
