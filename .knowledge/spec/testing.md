...
> **Read both dates before trusting the "cannot run here" rows.** This page is the *shell-era* strategy and most
> of it measures the closed E2B box. The 2026-09-19 host change is behind
> [pixi-pack §16.8](/research/pixi-pack.md#168-measured-2026-09-19-the-full-loop-runs-end-to-end-on-this-host):
> `pixi`/`cargo`/`pixi-pack` now install and run on the authoring host, so **row six of the table below**
> (`pixi`/`pixi-pack`/`cargo` ❌) is historical — the *airlocked target's* "no compiler" constraint is unchanged,
> which is why the [D21 test plan](/spec/testing-strategy.md) keeps L2's drivers stubbed. The D21 test plan
> supersedes this page for the Rust era.
