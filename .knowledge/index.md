...
>
> **Host change (2026-09-19):** the paragraph above is *dated* — it describes the closed E2B box. The current
> authoring host has open egress and runs `pixi`, `pixi-pack`/`pixi-unpack` and `rustc`/`cargo` from conda-forge,
> and the full `pixi → pixi-pack → file:// channel → pixi-unpack` loop was measured end-to-end
...
> The *design target* (a git-only airlock) is unchanged.
