...
`.partNNN` case, a root `pixi-sandbox` stub, and `restore.sh`/`restore.ps1` launchers that call it.
The root stub is intentionally outside the manifest; the nested tool stub remains the verified payload.
